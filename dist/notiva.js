...
(function (root, factory) {
  ...
  function getContainer(position) {
    ...
    if (!containers[pos] || !document.body.contains(containers[pos])) {
      var container = document.createElement('div');
      container.className = 'ss-container ss-pos-' + pos;
      container.setAttribute('data-position', pos);

      // Mouse enter / leave for Hover expand with debounce
  ...
  function showBackdrop(isBlur, customColor) {
    ...
    activeBackdrops++;
    if (isBlur) {
      globalBackdropEl.classList.add('ss-backdrop-blur');
    }
    if (customColor) {
      globalBackdropEl.style.backgroundColor = customColor;
    }
    // trigger reflow
  ...
  // Re-calculate Stacking and Offsets (Stacking physics)
  ...
  function createToast(options) {
    ...
    if (options.action || options.cancel) {
      ...
      if (options.action) {
        actionsHtml += '<button type="button" class="ss-btn ss-btn-primary ss-action-btn">' + (options.action.label || 'Action') + '</button>';
      }
      if (options.cancel) {
        actionsHtml += '<button type="button" class="ss-btn ss-btn-cancel ss-cancel-btn">' + (options.cancel.label || 'Batal') + '</button>';
      }
      actionsHtml += '</div>';
    }
    ...
    // Prepend to array (newest is index 0)
  ...
  function enableDrag(el, id, pos) {
    ...
    function onPointerMove(e) {
      ...
      // Allow dragging in X and subtle Y
  ...
  /**
   * Universal Object-Style / Notiva-Style API Methods
   */
  function toast(arg1, arg2, arg3) {
    var opts = parseArgs(arg1, arg2, arg3);
  ...
  toast.loading = function (arg1, arg2, arg3) {
    ...
    if (opts.duration === undefined && opts.timer === undefined) {
      opts.duration = 0; // Infinite until dismissed
    }
    return createToast(opts);
  };
  ...
  // Auto-trigger Laravel Session Flash Toasts if embedded
  if (typeof window !== 'undefined') {
    document.addEventListener('DOMContentLoaded', function () {
      if (window.__LARAVEL_TOAST__) {
        var flash = window.__LARAVEL_TOAST__;
        if (Array.isArray(flash)) {
          flash.forEach(function (item) { toast.fire(item); });
        } else {
          toast.fire(flash);
        }
      }
